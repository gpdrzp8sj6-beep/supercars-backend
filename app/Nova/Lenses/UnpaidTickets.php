...
class UnpaidTickets extends Lens
{
    /**
    ...
    public static function query(LensRequest $request, $query)
    {
        return $request->withOrdering($request->withFilters(
            $query->where('status', '!=', 'completed')
                  ->whereHas('giveaways') // Only orders that have tickets assigned
                  ->with(['giveaways', 'user'])
    ...
    public function fields(NovaRequest $request)
    {
        return [
            ...
            Text::make("Checkout ID", 'checkoutId'),
    ...
    {
        return [
            new \App\Nova\Metrics\UnpaidTicketsValue,
            new \App\Nova\Metrics\UnpaidTicketsCount,
        ];
    }
